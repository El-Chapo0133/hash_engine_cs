...
namespace Project.engines.hash
{
    class Hash {
        ...
        private int[] parse(string chain) {
            ...
            for (int index = 0; idnex < chain.length; index += 2) {
                return_list.add(Convert.ToInt32(appendValue(chain, index)));
            }
            return return_list.toArray();
        }
        private string appendValue(chain, index) {
            string temp_value = chain[index]
        ...
        private char convertStringOneValue(value) {
            return (char)value;
        }
    ...
    class Hash_Return {
    	...
    	contructor(string hash, bool isRight) {
    		this.hash = hash;
